...
@Component({
	selector: 'app-scholar-family-manager-modify',
	templateUrl: './scholar-family-manager-modify.component.html',
	styleUrls: ['./scholar-family-manager-modify.component.css']
})
export class ScholarFamilyManagerModifyComponent implements OnInit {
	...
	constructor(private scholarFamilyService:ScholarFamilyService,
		...
		this.scholarFamily = this.navParams.get('scholarFamily');
	...
	public save(scholarFamily:ScholarFamily) {
		...
		if (this.scholarFamily.id === 0) {
			this.scholarFamilyService.save(scholarFamily).subscribe((scholarFamilies) => {
				this.modalController.dismiss({ 'dismissed': true, 'scholarFamilies':scholarFamilies });
			}, (error) => {
					this.errorMessage = error.message;
			})
		} else {
