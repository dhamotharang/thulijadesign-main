...
@Component({
	selector: 'app-trainer-family-manager-modify',
	templateUrl: './trainer-family-manager-modify.component.html',
	styleUrls: ['./trainer-family-manager-modify.component.css']
})
export class TrainerFamilyManagerModifyComponent implements OnInit {
	...
	constructor(private trainerFamilyService:TrainerFamilyService,
		...
		this.trainerFamily = this.navParams.get('trainerFamily');
	...
	public save(trainerFamily:TrainerFamily) {
		...
		if (this.trainerFamily.id === 0) {
			this.trainerFamilyService.save(trainerFamily).subscribe((trainerFamilies) => {
				this.modalController.dismiss({ 'dismissed': true, 'trainerFamilies':trainerFamilies });
			}, (error) => {
					this.errorMessage = error.message;
			})
		} else {
