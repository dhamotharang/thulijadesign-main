...
@Component({
	selector: 'app-scholar-occupation-modify',
	templateUrl: './scholar-occupation-modify.component.html',
	styleUrls: ['./scholar-occupation-modify.component.scss']
})
export class ScholarOccupationModifyComponent implements OnInit {
	...
	constructor(private scholarOccupationService:ScholarOccupationService,
		...
		this.scholarOccupation = this.navParams.get('scholarOccupation');
	...
	public save(scholarOccupation:ScholarOccupation) {
		...
		if (this.scholarOccupation.id === 0) {
			this.scholarOccupationService.save(scholarOccupation).subscribe((scholarOccupations) => {
				this.modalController.dismiss({ 'dismissed': true, 'scholarOccupations':scholarOccupations });
			}, (error) => {
					this.errorMessage = error.message;
			})
		} else {
